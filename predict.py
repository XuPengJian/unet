...
def parse_args():
    ...
    parser.add_argument('--weights_path', type=str,
                        default=r"./save_weights/best_model.pth",
                        help='模型权重路径')
    parser.add_argument('--image_path', type=str,
                        default=r"D:\gitlab\road_damage_detection\yolov5\predict_result\img_crop",
                        help='检测图片的文件夹路径')
...
def main(args):
    ...
    for filename in os.listdir(args.image_path):
        ...
        if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png'):
            # 拼接文件路径
            img_path = os.path.join(args.image_path, filename)
            # load image
            ...
            with torch.no_grad():
                ...
                mask.save(os.path.join(args.save_path, filename))
